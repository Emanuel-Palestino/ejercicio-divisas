...
const usd = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    // Obtener la fecha de la petición
    const formatedDate = (0, helpers_1.getSQLDateFormat)(new Date());
    try {
        // Verificar si existe una entrada en la base de datos tipo_cambio con la misma fecha y el tipo de cambio USD
        const dbResponse = yield database_1.client.query(`SELECT * FROM tipo_cambio WHERE fecha = '${formatedDate}' AND tipo = 'USD'`);
        if (dbResponse.rows.length > 0) { // si existe una entrada en la base de datos
            ...
            // Obtener el tipo de cambio de MXN a USD
            const response = yield fetch(`${constants_1.USD_URI}/${formatedDate}/${formatedDate}`, {
                headers: {
            ...
            yield database_1.client.query(`INSERT INTO tipo_cambio (fecha, tipo, cambio) VALUES ('${formatedDate}', 'USD', ${values.bmx.series[0].datos[0].dato})`);
...
exports.usd = usd;
//# sourceMappingURL=usd.controller.js.map