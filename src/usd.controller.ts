import { USD_URI, BMX_TOKEN } from './constants'
import { getSQLDateFormat } from './helpers'
import { client } from './database'
import { Request, Response } from 'express'


export const usd = async (req: Request, res: Response) => {
	...
	try {
		...
		const dbResponse = await client.query(`SELECT * FROM tipo_cambio WHERE fecha = '${formatedDate}' AND tipo = 'USD'`)
		...
		if (dbResponse.rows.length > 0) { // si existe una entrada en la base de datos
			...
			const response = await fetch(`${USD_URI}/${formatedDate}/${formatedDate}`, {
				headers: {
					'Bmx-Token': BMX_TOKEN
				}
			})
			...
			await client.query(`INSERT INTO tipo_cambio (fecha, tipo, cambio) VALUES ('${formatedDate}', 'USD', ${values.bmx.series[0].datos[0].dato})`)
